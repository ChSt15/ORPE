...
void PoseEstimator::estimatePose()
{
    ...
    if (pointsImage.size() == satellite_.ledModel.size()*2 && dynamicThres_ < 250) {
	dynamicThres_++;
    } else if (dynamicThres_ > 5) {
	dynamicThres_--;
    }
    ...
    if (validModelPoints.size() >= 4)
    {
        ...
        solveOk = cv::solvePnPRansac(cv::Mat(convertLEDTo3dPoint(validModelPoints)), cv::Mat(convertLEDTo2dPoint(validImagePoints)), cameraMatrix_, cameraDistorionMatrix_, rotM, tranM, false, 100, 8.0, 0.99, cv::noArray(), cv::SolvePnPMethod::SOLVEPNP_IPPE/*SOLVEPNP_EPNP*/);
