...
void PoseEstimator::estimatePose()
{
    ...
    auto pointsImage = getPointsInImage(currentImage_, 200);
    ...
    if (validModelPoints.size() >= 4)
    {
        ...
        solveOk = cv::solvePnPRansac(cv::Mat(convertLEDTo3dPoint(validModelPoints)), cv::Mat(convertLEDTo2dPoint(validImagePoints)), cameraMatrix_, cameraDistorionMatrix_, rotM, tranM, false, 100, 8.0, 0.99, cv::noArray(), cv::SolvePnPMethod::SOLVEPNP_EPNP);
        ...
        else if ((sqrt(rotDiff) > 10*3.14/180 || sqrt(tranDiff) > 50) && estimationCounter_ > 0) //If difference between prediction and estimation is greater than 5 deg or 2 cm and there has been a good solve
            solveOk = false;
